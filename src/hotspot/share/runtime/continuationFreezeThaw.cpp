...
extern uint64_t _last_write_time;
...
static void jvmti_mount_end(JavaThread* current, ContinuationWrapper& cont, frame top, bool has_oop_on_stub) {
  assert(java_lang_VirtualThread::notify_jvmti_events(), "must be");
  ...
  if (_global_logging) {
    log_trace(continuations, constantpool)("before posting mount even in thaw at time: " INT64_FORMAT "\n", (int64_t)os::javaTimeNanos());
  }
  ...
  JRT_BLOCK
    ...
    if (_global_logging) {
      //log_trace(continuations, constantpool)("passed finish_VTMS_transition at time: " INT64_FORMAT "\n", (int64_t)os::javaTimeNanos());
    }
    ...
    if (_global_logging) {
      //log_trace(continuations, constantpool)("passed post_vthread_mount at time: " INT64_FORMAT "\n", (int64_t)os::javaTimeNanos());
    }
  JRT_BLOCK_END
  ...
  if (_global_logging) {
    //log_trace(continuations, constantpool)("exiting jvmti_mount_end at time: " INT64_FORMAT "\n", (int64_t)os::javaTimeNanos());
  }
}
#endif // INCLUDE_JVMTI
...
NOINLINE intptr_t* ThawBase::thaw_slow(stackChunkOop chunk, bool return_barrier) {
  ...
  if (!return_barrier && _cont.is_preempted()) {
    ...
    JVMTI_ONLY(if (_thread->is_in_VTMS_transition()) jvmti_mount_end(_thread, _cont, top, has_oop_on_stub);)

    if (top_is_interpreted) {
      sp = push_preempt_rerun_adapter(top, true /* is_interpreted_frame */);
      if (_global_logging) {
        log_trace(continuations, constantpool)("thaw with interpreted frame on top at time: " INT64_FORMAT "\n", (int64_t)os::javaTimeNanos());
      ...
      if (_global_logging) {
        log_trace(continuations, constantpool)("thaw with safepoint blob frame on top at time: " INT64_FORMAT "\n", (int64_t)os::javaTimeNanos());
        _last_write_time = os::javaTimeNanos();
      }
    }
  }
