...
freeze_result FreezeBase::finalize_freeze(const frame& callee, frame& caller, int argsize_md) {
  ...
  if (_preempt && _thread->return_oop() != nullptr) {
    ...
    log_trace(continuations, preempt)("Preempting with return oop on stub");
...
NOINLINE intptr_t* ThawBase::thaw_slow(stackChunkOop chunk, bool return_barrier) {
  ...
  if (!return_barrier && _cont.is_preempted()) {
    ...
    if (Interpreter::contains(top.pc())) {
      ...
      if (_cont.tail() != nullptr && _cont.tail()->has_oop_on_stub()) {
        _cont.tail()->set_has_oop_on_stub(false);
      }
    }
  }
